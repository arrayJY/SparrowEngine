...
{
    auto appInfo = vk::ApplicationInfo().setPApplicationName("Sparrow").setApplicationVersion(VK_MAKE_VERSION(1, 0, 0))
            .setPEngineName("Sparrow Engine").setEngineVersion(VK_MAKE_VERSION(1, 0, 0)).setApiVersion(VK_VERSION_1_3);

    static const std::vector<const char*> validationLayers{"VK_LAYER_KHRONOS_validation", "VK_LAYER_LUNARG_monitor"};
    ...
    auto createInfo = vk::InstanceCreateInfo().setPApplicationInfo(&appInfo).setPEnabledExtensionNames(validationLayers)
...
void VulkanRHI::setupDebugMessenger()
{
    ...
    auto debugMessengerInfo = vk::DebugUtilsMessengerCreateInfoEXT().setMessageSeverity(
                    SeverityFlag::eInfo | SeverityFlag::eVerbose | SeverityFlag::eWarning | SeverityFlag::eError)
            .setMessageType(MessageType::eGeneral | MessageType::eValidation | MessageType::ePerformance)
            .setPfnUserCallback(callbackFunc).setPUserData(nullptr);
    ...
    if (instance.createDebugUtilsMessengerEXT(&debugMessengerInfo, nullptr, &debugMessenger, dispatch)
            !=vk::Result::eSuccess) {
...
{
    auto surfaceInfo = vk::Win32SurfaceCreateInfoKHR().setHwnd(hwnd).setHinstance(hInstance);
...
void VulkanRHI::createLogicalDevice()
{
    ...
    for (auto queueFamilyIndex : uniqueQueueFamilies) {
        auto deviceQueueInfo = vk::DeviceQueueCreateInfo().setQueueCount(1).setPQueuePriorities(&priorities)
                .setQueueFamilyIndex(queueFamilyIndex);
        queueCreateInfos.push_back(deviceQueueInfo);
    }

    auto deviceInfo = vk::DeviceCreateInfo().setQueueCreateInfos(queueCreateInfos)
            .setPEnabledExtensionNames(deviceExtensions).setPEnabledFeatures(&feature);
...
{
    auto commandPoolInfo = vk::CommandPoolCreateInfo().setFlags(vk::CommandPoolCreateFlagBits::eResetCommandBuffer)
...
{
    auto allocInfo = vk::CommandBufferAllocateInfo().setCommandPool(commandPool)
            .setLevel(vk::CommandBufferLevel::ePrimary).setCommandBufferCount(1);
...
void VulkanRHI::createSwapChain()
{
    ...
    auto swapChainInfo = vk::SwapchainCreateInfoKHR().setSurface(surface).setImageFormat(format)
            .setMinImageCount(imageCount).setImageColorSpace(swapChainImageFormat.colorSpace)
            .setImageExtent(swapChainExtent).setImageArrayLayers(1)
            .setImageUsage(vk::ImageUsageFlagBits::eColorAttachment).setImageSharingMode(imageSharingMode)
            .setQueueFamilyIndexCount(queueFamilyIndexCount).setPQueueFamilyIndices(pQueueIndices)
...
void VulkanRHI::createSwapChainImageView()
{
    ...
    for (uint32_t i = 0; i<frameCount; i++) {
        auto createImageViewInfo = vk::ImageViewCreateInfo().setImage(swapChainImages[i])
                .setViewType(vk::ImageViewType::e2D).setFormat(format).setComponents(
...
void VulkanRHI::createFramebufferImageAndView()
{
    VulkanUtils::createImage(gpu, device, swapChainExtent.width, swapChainExtent.height, format,
            ...
            vk::ImageUsageFlagBits::eInputAttachment | vk::ImageUsageFlagBits::eDepthStencilAttachment
                    | vk::ImageUsageFlagBits::eTransferSrc, vk::MemoryPropertyFlagBits::eDeviceLocal, std::nullopt, 1,
            1, depthImage, depthDeviceMemory);
...
bool VulkanRHI::checkValidationLayerSupport(const std::vector<const char*>& layerNames)
{
    ...
    return std::all_of(layerNames.begin(), layerNames.end(), [&properties](auto layerName) {
      return std::any_of(properties.begin(), properties.end(), [layerName](const vk::LayerProperties& prop) {
        ...
        void* pUserData)
{
    ...
    case DebugSeverityFlag::eInfo:std::cout << "[validation layer (Info)]" << pCallbackData->pMessage << "\n";
        break;
    case DebugSeverityFlag::eVerbose:std::cout << "[validation layer (Verbose)]" << pCallbackData->pMessage << "\n";
        break;
    case DebugSeverityFlag::eWarning:std::cerr << "[validation layer (Warning)]" << pCallbackData->pMessage << "\n";
        break;
    default:std::cerr << "[validation layer (Error)]" << pCallbackData->pMessage << "\n";
        break;
    }
    return VK_FALSE;
}
...
{
    return SwapChainSupportDetails{.capabilities = device.getSurfaceCapabilitiesKHR(surface), .formats = device
            .getSurfaceFormatsKHR(surface), .presentModes = device.getSurfacePresentModesKHR(surface),};
}
